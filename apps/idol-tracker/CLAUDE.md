...
- Pages はモノレポのためブランチ配信の `/docs` が使えず、`idol-tracker-pages.yml` で `docs/` をアーティファクト配信する（Settings → Pages → Source は「GitHub Actions」。enablement による自動有効化は GITHUB_TOKEN の権限では不可）
