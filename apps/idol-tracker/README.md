...
1. **Settings → Pages** で Source に「GitHub Actions」を選びます（この操作だけは画面から行う必要があります。`configure-pages` の `enablement` による自動有効化は `GITHUB_TOKEN` の権限では失敗します）。`idol-tracker pages` ワークフローが `apps/idol-tracker/docs` だけを配信するので、数分で `https://<ユーザー名>.github.io/inventory/` が開けるようになります（モノレポではブランチ配信の `/docs` が使えないため、この方式です）。
